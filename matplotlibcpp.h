...
bool plot_base(PyObject* const                           pyfunc,
               const VectorX&                            x,
               const VectorY&                            y,
               const std::string&                        s        = "",
  ...
  for (auto const& item : keywords) {
    PyDict_SetItemString(
        kwargs, item.first.c_str(), PyString_FromString(item.second.c_str()));
  }
...
template<typename VectorX, typename VectorY>
bool plot(const VectorX&                            x,
          const VectorY&                            y,
          const std::string&                        s        = "",
          const std::map<std::string, std::string>& keywords = {}) {
  return detail::plot_base(
      detail::_interpreter::get().s_python_function_plot, x, y, s, keywords);
}
...
template<typename VectorX, typename VectorY>
bool loglog(const VectorX&                            x,
            const VectorY&                            y,
            const std::string&                        s        = "",
            const std::map<std::string, std::string>& keywords = {}) {
  return detail::plot_base(
      detail::_interpreter::get().s_python_function_loglog, x, y, s, keywords);
}
...
// @param keywords Additional keywords
template<typename Matrix>
void plot_surface(const Matrix&                             x,
                  const Matrix&                             y,
                  const Matrix&                             z,
                  const std::map<std::string, std::string>& keywords =
...
bool _plot(const A& a, const B& b, const std::string& format, Args... args) {
  return plot(a, b, format) && _plot(args...);
}
